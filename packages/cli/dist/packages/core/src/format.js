...
exports.formatInspection = formatInspection;
const config_1 = require("./config");
const count = (value) => value.toLocaleString('en-US');
const plural = (value, singular, pluralForm = `${singular}s`) => `${count(value)} ${value === 1 ? singular : pluralForm}`;
const MAX_FIELDS_PER_COLLECTION = 12;
const MAX_WARNINGS = 50;
function formatInspection(report, options = {}) {
    const firestore = report.sources?.firestore ?? { ok: true };
    const auth = report.sources?.auth ?? { ok: true };
    const lines = [options.projectId ? `FireMigrate inspect - project ${options.projectId}` : 'FireMigrate inspect', ''];
        ...
        for (const collection of report.collections) {
            ...
            for (const field of shown) {
                ...
                if (field.types.length > 1)
                    notes.push('⚠ mixed types');
                if (field.relationship)
                    notes.push(field.relationship.confidence < 0.85 ? `⚠ may reference "${field.relationship.targetCollection}"` : `→ ${field.relationship.targetCollection}`);
                if (field.presenceRate < 1 && field.types[0] !== 'null')
                    notes.push(`in ${Math.round(field.presenceRate * 100)}% of sampled docs`);
                lines.push(`      ${field.name.padEnd(width)}  ${field.types.join(' | ')}${notes.length ? `   ${notes.join('  ')}` : ''}`);
            }
            if (collection.fields.length > shown.length)
                lines.push(`      ... ${collection.fields.length - shown.length} more fields`);
        }
    }
    if (auth.ok) {
        ...
        if (providers.length)
            lines.push(`  providers: ${providers.map(([name, value]) => `${name} ${count(value)}`).join(' · ')}  (a user can have several)`);
        if (report.auth.userCount > 0 && report.auth.providers.password > 0)
    ...
    if (mixed || review)
        lines.push('');
    if (mixed)
        lines.push(`⚠ ${plural(mixed, 'collection')} ${mixed === 1 ? 'contains' : 'contain'} inconsistent field types`);
    if (review)
        lines.push(`⚠ ${plural(review, 'relationship')} ${review === 1 ? 'requires' : 'require'} manual review`);
    if (report.warnings.length) {
        lines.push('', 'Details');
        for (const warning of report.warnings.slice(0, MAX_WARNINGS))
            lines.push(`  - ${warning}`);
        if (report.warnings.length > MAX_WARNINGS)
            lines.push(`  ... ${report.warnings.length - MAX_WARNINGS} more`);
    }
    lines.push('');
    if (!firestore.ok)
        lines.push('Fix the errors above and run inspect again. No schema can be generated without Firestore access.');
    else if (report.collections.length === 0)
        lines.push('No collections were found, so there is nothing to migrate.');
    else
        lines.push(`Ready to generate a PostgreSQL schema: ${config_1.PACKAGE_RUNNER} schema`);
    return `${lines.join('\n')}\n`;
}
